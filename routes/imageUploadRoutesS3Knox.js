    ...
    .post( uploader.single('file'), (req, res) => {
        if (req.file) {
            s3.makeS3Request(req, res, () => {
                db.saveImageS3UrlToDb(req.file.filename, req.session.user.userId).then(function(result) {
                    res.json({
                        success: true,
                        file: result
                    });
...
router.route('/uploadImageForFeed')
    ...
    .post( uploader.single('file'), (req, res) => {
        if (req.file) {
            s3.makeS3Request(req, res, () => {
                db.savePostWithS3ImageToDb(req.session.user.userId, req.body.message, req.file.filename).then(function(result) {
                    res.json({
                        success: true,
                        post: result
                    });
